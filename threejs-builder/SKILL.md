...
Three.js work is scene-graph work plus rendering verification. Every visible result depends on four contracts being correct: module loading, camera/framing, lighting/materials, and object transforms.
...
5. Visual polish that fits the requested scene, game, or product viewer
...
| Game patterns | [game-patterns.md](references/game-patterns.md) | Building Three.js games, animation state machines, fixed cameras, object pools, time scaling, and terminal states |
...
1. Discover the project shape before editing.
   ...
   - Inspect package scripts and existing imports with `rg -n "from ['\"]three|GLTFLoader|OrbitControls|WebGLRenderer|setAnimationLoop|requestAnimationFrame" .`.
...
2. Choose the smallest durable implementation path.
   ...
   - Game: define state, input, update loop, camera convention, and terminal latches before adding effects.
   - GLTF work: calibrate one model first, then scale to many models.

3. Build the scene contract.
   - Renderer: antialias only when needed, `setPixelRatio(Math.min(devicePixelRatio, 2))`, resize handling, and `outputColorSpace = THREE.SRGBColorSpace`.
   - Camera: position, target, near/far planes, and responsive aspect/frustum update.
...
4. Implement interaction and animation.
   - Use `renderer.setAnimationLoop` for render loops.
...
- Game: fixed or constrained camera, snappy input, state machine, pooled objects, clear collision/debug views
...
- Interaction checks: orbit/pointer/keyboard/touch behavior matches the request
