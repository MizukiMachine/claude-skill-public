...
`gpt-image-2` is the newer, higher-quality GPT Image model and is the default choice when the user just wants the best or latest OpenAI image model. Use `gpt-image-1-5` only when the user explicitly asks for `gpt-image-1.5` or needs transparent backgrounds, which `gpt-image-2` does not support. For Open Graph / social-share images specifically, use the dedicated `og-image-ai` pipeline instead.
