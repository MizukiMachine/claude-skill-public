...
Examples:
  uv run scripts/asset_manifest_export_json.py \\
    --manifest assets/rocky_roads/assets_index.lua \\
    --out assets/rocky_roads/assets_index.json
"""
    ...
    def _parse_table(self) -> Any:
        ...
        while self._cur().t != "}":
            ...
            if self._cur().t in (",", ";"):
                ...
                # allow trailing separators or end of table
                pass

        self._eat("}")

        # Convert to list if it's a pure 1..n array; else dict.
        ...
        for k, v in obj.items():
            if k == "path" and isinstance(v, str) and v.lower().endswith(".png"):
                ...
                if not p.is_absolute():
                    # Interpret relative paths as relative to CWD for legacy Lua manifests.
...
def export_manifest(manifest_path: Path, pack_relative: bool) -> dict[str, Any]:
    ...
    if pack_relative:
        # Make the manifest self-contained: meta.root="." and all asset paths relative
        # to the folder containing the output manifest.
