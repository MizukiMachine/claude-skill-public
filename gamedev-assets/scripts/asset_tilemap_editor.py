...
Examples:
  uv run scripts/asset_tilemap_editor.py \\
    --manifest assets/rocky_roads/assets_index.json
...
def _resolve_asset_path(manifest_path: Path, manifest: dict[str, Any], rel: str) -> Path:
    # Prefer: manifest_dir / meta.root / rel
    ...
    for c in candidates:
        if c.exists():
            return c
    # Fall back to the most likely location for error messages.
    return candidates[0]
...
class EditorApp:
    ...
    def _bind_keys(self) -> None:
        ...
        # Ctrl+S / Ctrl+O / Ctrl+L are not reliably delivered on all platforms,
        # but are still convenient when they work (Windows/Linux).
        ...
        if self.grid:
            for c in range(meta.columns + 1):
                ...
                self._palette_grid_ids.append(
                    self._palette_canvas.create_line(x, 0, x, meta.rows * meta.tile_h * self.palette_scale, fill="#ffffff", stipple="gray25")
                )
            for r in range(meta.rows + 1):
                y = r * meta.tile_h * self.palette_scale
                self._palette_grid_ids.append(
                    self._palette_canvas.create_line(0, y, meta.columns * meta.tile_w * self.palette_scale, y, fill="#ffffff", stipple="gray25")
                )
    ...
    def _redraw_map(self) -> None:
        ...
        if self.grid:
            # Light grid only in viewport.
            for c in range(start_c, end_c + 1):
                x = c * tile_px - self.cam_x
                self._map_grid_ids.append(self._map_canvas.create_line(x, 0, x, view_h, fill="#ffffff", stipple="gray25"))
            for r in range(start_r, end_r + 1):
                y = r * tile_py - self.cam_y
                self._map_grid_ids.append(self._map_canvas.create_line(0, y, view_w, y, fill="#ffffff", stipple="gray25"))
    ...
    def _draw_help(self) -> None:
        ...
        for i, line in enumerate(lines):
            self._map_canvas.create_text(x + 10, y + 10 + i * 18, anchor="nw", text=line, fill="#e8e8e8", font=("Menlo", 12))
